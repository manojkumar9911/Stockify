...
def BuyPage(request):
    ...
    if (market_open <= ind_time <= market_close) and (
...
def SellPage(request):
    ...
    for l in range(len(profit)):
        if cpintoquan[l] != 0:
            ...
            percent.append(float("inf"))  # Handle division by zero case
...
def portfolio(request):
    ...
    for i in range(len(profit)):
        if currentvalue[i] != 0:
            result = round((profit[i] * 100) / currentvalue[i], 2)  # Round to
            percent.append(result)
        else:
            percent.append(0)  # Handle division by zero case
    ...
    jandata = Portfolio.objects.filter(uname=name, month="2024-01")
    for i in jandata:
        l = i.quantity * i.buyPrice
        janinv.append(l)

    febdata = Portfolio.objects.filter(uname=name, month="2024-02")
    for i in febdata:
        l = i.quantity * i.buyPrice
        febinv.append(l)

    mardata = Portfolio.objects.filter(uname=name, month="2024-03")
    for i in mardata:
        l = i.quantity * i.buyPrice
        marinv.append(l)

    aprdata = Portfolio.objects.filter(uname=name, month="2024-04")
    for i in aprdata:
        l = i.quantity * i.buyPrice
        aprinv.append(l)

    maydata = Portfolio.objects.filter(uname=name, month="2024-05")
    for i in maydata:
        l = i.quantity * i.buyPrice
        mayinv.append(l)

    jundata = Portfolio.objects.filter(uname=name, month="2024-06")
    for i in jundata:
        l = i.quantity * i.buyPrice
        juninv.append(l)
    ...
    for item in investedvalue:
        totalInvested = totalInvested + item

    totalCurrent = 0
    for item in currentvalue:
        totalCurrent = totalCurrent + item

    totalInvested = round(totalInvested,3)
    totalCurrent = round(totalCurrent,3)
